...
mod packing_tests {
    ...
    fn pool(density: usize) -> PoolLoad {
        PoolLoad {
            isolates: vec![],
            limits: PoolLimits { grow_at: 2, shrink_under: 1, max_stateless: 4,
                max_requests: None, max_cells: density },
        }
    }
        ...
        for density in [1, 2, 4, 32] {
            ...
            for cells in 1..=40 {
                match isolate::place_cell(&load) {
                    Placement::Grow => load.isolates.push(IsolateLoad { cells: 1, ..Default::default() }),
    ...
    fn packing_still_fills_live_heaps_and_leaves_retiring_heaps_alone() {
        let mut load = pool(2);
        load.isolates = vec![
            IsolateLoad { retiring: true, ..Default::default() },
            IsolateLoad { cells: 1, ..Default::default() },
            IsolateLoad::default(),
        ];
    ...
    fn retirement_requires_empty_heaps_and_freeing_requires_drained_requests() {
        let mut load = pool(1);
        load.isolates = vec![IsolateLoad { cells: 1, ..Default::default() }];
